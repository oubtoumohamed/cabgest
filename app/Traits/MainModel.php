...
trait MainModel {
    ...
    public static function scopeList($query)
    {
        if( request('forAction') == "loadSelect" )
            return $query->select( self::selectable() )->filter()->orderBy( self::getOrderbyColumn() , self::getOrderbyDirection() )->get();
    ...
    public static function refgenerate($from, $field='numero'){

        $ref = date('ymd') . str_pad($from+1, 3, '0', STR_PAD_LEFT);

        $ref_check = self::where($field,$ref)->first();

        if($ref_check){
            return  self::refgenerate($from+1);
        }
